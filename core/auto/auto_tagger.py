...
3. auto_keywords: doc title + content 의 핵심 명사 5~10개 배열. 띄어쓰기 포함 가능. 사용자가 자연어로 물어볼 때 매칭될 단어 위주.
   ...
   ★★ 2글자 bare 토큰 분리 추출 (필수):
   - 섹션 제목/합성어에 핵심 도메인 명사가 들어있으면, **그 2글자 명사를 단독 토큰으로 별도 추출**
   - 예: "구매 업무 지침" → ["구매 업무", "구매", "발주"], "출장비 규정" → ["출장비", "출장"],
        "택시비" → ["택시비", "택시"], "징계기준" → ["징계기준", "징계"],
        "계약 검토" → ["계약 검토", "계약"], "정산 절차" → ["정산 절차", "정산"]
   - 사용자가 짧은 2글자 자연어("출장", "택시", "징계")로 물어볼 때 매칭되도록 함
   - 일반 행정어 노이즈 가드(제외): "관리", "운영", "업무", "제도", "절차", "기준", "지침",
     "보고", "신청", "처리", "등록", "조회", "확인", "검토", "승인" (단독 추출 금지 — 합성어 OK)
...
★★ 2글자 bare 토큰 분리 추출 (필수):
- chunk 본문/섹션 제목에 핵심 도메인 명사가 합성어로 들어있으면, **그 2글자 명사를 단독 토큰으로 별도 추출**
- 예: "구매 업무" → ["구매 업무", "구매", "발주"], "출장비" → ["출장비", "출장"],
     "택시비" → ["택시비", "택시"], "징계기준" → ["징계기준", "징계"],
     "계약 검토" → ["계약 검토", "계약"], "정산" → ["정산"]
- 사용자가 짧은 2글자 자연어로 물어볼 때 매칭되도록 함
- 일반 행정어 노이즈 가드(제외): "관리", "운영", "업무", "제도", "절차", "기준", "지침",
  "보고", "신청", "처리", "등록", "조회", "확인", "검토", "승인" (단독 추출 금지 — 합성어 OK)
