...
AMBIGUOUS_BARE_TOKENS: dict[str, dict] = {
    ...
    "감사": {
        ...
        "choices": [
            ...
            # AEO 감사 선택지 제거: 영업 AEO 사규 10종에 감사 관련 내용이
            # 곁가지로만 존재(위험관리규정의 연 1회 위험평가 수준) — 내부회계·
            # 정보보호 같은 전용 감사 사규 부재. 메이저 선택지와 나란히 버튼으로
            # 올리면 마이너 주제를 과대 노출 = false-negative 약속. 직접
            # "AEO 위험평가"/"AEO 내부통제활동" 질의 시 full RAG 가 위험관리규정
            # 찾아 정상 답하므로 정보 손실 없음. (감사위원회 제거와 동일 논리)
            # 감사위원회 선택지도 제거: 독립 "감사위원회 운영규정" 사규가 corpus 에
