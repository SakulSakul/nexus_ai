...
5. out_of_scope: 사규/윤리/컴플라이언스 범위를 벗어난 query — 검색·생성을
   ...
   인사기록, 인사 행정(휴가·연차·근태·인사평가·승진·채용·복리후생),
   잡담/테스트, 타사 사규 등 단순 행정·외부 문의.
   단, 직장 내 괴롭힘·성희롱 예방/대응·징계 기준은 컴플라이언스라 in-scope
   (critical/standard 우선) — 위 인사 행정에서 제외.
   Positive 예: "VPN 접속 장애"(IT), "내 연봉 알려줘"(개인), "롯데면세점 사규"(타사),
       "오늘 날씨"(일반상식), "안녕"/"hello"/"테스트"(잡담), "회의실 예약"(총무),
       "휴가 가고 싶어"/"연차 신청"/"근태 정정"(인사 행정 → 인사교육팀)
   ...
   ⭐ 핵심 판별 (OOS vs standard):
     [OOS]      = 사규에 명시되지 않은 단순 행동 (회의실 예약·VPN 접속·
                  노트북 교체·주차 등록·구내식당·연봉 조회·휴가/연차/근태·
                  인사평가·잡담·타사 사규).
