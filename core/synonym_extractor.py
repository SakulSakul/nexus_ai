...
2. 사내 정식명 ↔ 약칭 (사규에 '이하 ...라 한다' 로 명시된 1:1 매핑):
   - "통합물류창고(센터): ... 이하 '통물' 이라 한다" → 통합물류창고(센터) ↔ 통물
   ※ 외부 법령(「…법률」/시행령/고시/…에 관한 규정)·일반명사(법/영/규정/지침/회사/당사)는 제외 — 아래 Reject 7·8.
...
7) **정의 조항 관용 일반명사 약칭 금지** (가장 중요):
   - 거의 모든 사규 정의 조항이 반복하는 boilerplate 약칭은 추출 금지.
   - 예: 법, 영, 령, 규정, 지침, 고시, 칙, 회사, 당사, 본사, 점포.
   - 일반명사를 전역 동의어로 등록하면 무관한 모든 query 를 오염시킴.

8) **외부 법령·시행령·고시 인용 금지**:
   - 정식명이 외부 법령이면 추출 금지 (본 챗봇은 내부 사규 전용).
   - 낫표 「 」 인용, 또는 정식명이 '법률/시행령/고시' 로 끝나거나 '…에 관한 규정' 형태.
   - 예: 「주식회사 등의 외부감사에 관한 법률」, 외부감사 및 회계 등에 관한 규정, 동법 시행령.

9) **영문 일반 번역어 금지**:
   - "한글(English)" 에서 영어가 사규 고유 약어가 아닌 일반 사전적 번역이면 금지.
   - 예: 관찰(Observation), 질문(Inquiry), IP(Internet Protocol).

JSON 출력 (예시):
{{"pairs": [{{"primary": "통물", "synonym": "통합물류창고(센터)", "evidence": "통합물류창고(센터): ... 이하 '통물' 이라 한다"}}]}}
