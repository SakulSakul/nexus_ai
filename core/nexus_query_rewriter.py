...
_REWRITE_MODEL = get_secret("NEXUS_QUERY_REWRITE_MODEL", "gemini-3.1-flash-lite")
...
_NEXUS_NL_TO_REGULATORY = {
    ...
    "사고": ("안전사고", "재해"),
    "다친": ("산업재해", "안전"),
    "부상": ("산업재해", "안전", "초동조치", "응급조치", "병원후송", "안전사고"),
    ...
    "안전보건": ("안전보건", "재해예방", "산업안전"),
    # 공정거래 / 협력회사 도메인 — 사내 동의어: 협력사원=파견사원=판촉사원.
    # 정답 사규 "협력회사 판촉사원 파견 지침" 의 식별 토큰 "판촉사원" 으로
    # 확장해 표시광고/비용부담 지침과의 오매칭을 차단. value 는 기존 invariant
    # (단일 토큰) 준수 — to_tsquery 가 다단어에 syntax error 를 던지므로
    # "협력회사 판촉사원" 은 "협력회사"+"판촉사원" 으로 분해.
    "협력사원": ("판촉사원", "파견사원", "협력회사", "파견인력"),
    "파견사원": ("판촉사원", "협력사원", "협력회사", "파견인력"),
    "판촉사원": ("협력사원", "파견사원", "협력회사"),
}
...
_NEXUS_NL_TO_INCIDENT_NODES = {
    # 인사사고 / 안전 응급 도메인
    "다쳤":   [("인사사고", "고객상해"), ("인사사고", "직원상해"), ("안전사고", None), ("인사사고", "응급대응"), ("인사사고", "시설안전")],
    ...
    "사망":   [("인사사고", "사망"), ("근로자안전", "인사사고"), ("응급대응", None)],
    ...
    "접대":   [("조직관리", "횡령수수")],
    "횡령":   [("조직관리", "횡령수수"), ("횡령", "배임"), ("금전사고", "비위행위"), ("윤리보고", None)],
    "유용":   [("조직관리", "횡령수수")],
    ...
    # 근무기강 도메인
    "성희롱": [("근무기강", "성희롱"), ("성희롱", "윤리보고")],
    "성추행": [("근무기강", "성희롱"), ("인사사고", "성범죄"), ("성희롱", "윤리보고")],
    "폭언":   [("근무기강", "폭언폭력"), ("괴롭힘", "윤리보고")],
    "폭력":   [("근무기강", "폭언폭력")],
    "괴롭":   [("근무기강", "괴롭힘")],
    "갑질":   [("근무기강", "부당한지시"), ("근무기강", "괴롭힘"), ("괴롭힘", "윤리보고")],
    "결근":   [("근무기강", "근무태만")],
    "지각":   [("근무기강", "근무태만")],
    ...
    # 안전관리 도메인 (사고 유형)
    "화재":   [("안전관리", "화재사고"), ("안전사고", "화재"), ("매장사고", "응급대응")],
    ...
    "환경법규":        [("환경법규", None)],
    "환경 법규":       [("환경법규", None)],
    "환경경영":        [("환경관리", None), ("환경경영", None)],
    "환경 사고":       [("환경관리", "사건사고보고"), ("환경사고", None)],
    "오염":            [("환경관리", "환경법규")],
    "유해물질":        [("환경관리", "환경법규")],
    "화학물질":        [("환경관리", "환경법규")],
    "ISO 14001":      [("환경관리", None)],
    ...
    # PR-Phase-14.2-Fix-N: 비위/부정 발견·신고·제보 trigger — "비위사실 발견"
    # 등이 '발견' 단어에 끌려 (안전) 실종아동 발견 지침으로 오매칭되던 치명적
    # 검색 결함 fix. 윤리보고/윤리위반 매핑으로 (공통) 임직원 징계기준·클린뱅크
    # 등 컴플라이언스 사규 force-include.
    "비위사실":        [("윤리보고", "윤리위반"), ("비위행위", None)],
    "비위사실 발견":    [("윤리보고", "윤리위반"), ("비위행위", None)],
    "비위 발견":       [("윤리보고", "윤리위반")],
    "부정 발견":       [("윤리보고", "윤리위반")],
    "부정 적발":       [("윤리보고", "윤리위반")],
    "비위 신고":       [("윤리보고", "윤리위반")],
    ...
    "제보":           [("윤리보고", "윤리위반")],
    "내부 제보":       [("윤리보고", "윤리위반")],
    # PR-Phase-16.2-Fix: 일반 신고 방법 + 범죄/위반사실 trigger (Gap A + 카테고리).
    # nodes 부재로 Domain-Lock 스킵 → fallback noise(안전) 추락하던 결함 fix.
    # 도메인 결합 query(성희롱 신고 방법 등)는 Phase-16.1 도메인 우선으로 회귀 0,
    # 순수 일반 신고/범죄/위반 query 만 ethics 최후 fallback → 🤝 CSR.
    "신고 방법":       [("윤리보고", "윤리위반")],
    "신고 방법 안내":   [("윤리보고", "윤리위반")],
    "어떻게 신고":      [("윤리보고", "윤리위반")],
    "신고 절차":       [("윤리보고", "윤리위반")],
    "신고하려면":      [("윤리보고", "윤리위반")],
    "범죄사실":        [("윤리보고", "윤리위반"), ("비위행위", None)],
    "범죄 사실":       [("윤리보고", "윤리위반"), ("비위행위", None)],
    "위반사실":        [("윤리보고", "윤리위반"), ("비위행위", None)],
    "위반 사실":       [("윤리보고", "윤리위반"), ("비위행위", None)],
    "법규 위반":       [("윤리보고", "윤리위반"), ("비위행위", None)],

    # #4 부당 비용 — 임직원 징계기준 + 부당한 경영활동 지침 force-include.
    # (PR-Phase-14.1: 협력회사 중복 키 제거 — 위 공정거래 섹션 매핑 사용.)
    "부당 비용":       [("비위행위", None), ("윤리위반", None)],
    "비용 요구":       [("비위행위", None), ("윤리위반", None)],
    "부당하게":        [("비위행위", None), ("윤리위반", None)],

    # PR-Phase-14.1: '처벌'/'징계 수위' 강제 윤리위반 매핑 제거 — DB (공통)
    # 임직원 징계기준 사규가 자연 검색되도록 RAG 자연 정확도 의존.
